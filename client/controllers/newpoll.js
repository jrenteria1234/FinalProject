Template.newPoll.events({
  "click #preview": function(event){
    ...
    if (questionTextValue === "") {
      $('.questionMessage').html("Insert Institution!");
  ...
  "submit form": function(event){
    ...
    const institution = event.target.questionText.value;
    const className = event.target.classNameText.value;
    const classId = event.target.classIdText.value;
    ...
    // Session.set("institution", questionText);
    // Session.set("className", className);
    // Session.set("classId", classId);

    Meteor.call("addPoll", [institution, className, classId, choiceText]);
  }
});
