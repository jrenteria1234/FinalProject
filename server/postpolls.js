Meteor.methods({
  addPoll: function (data) {
    ...
    // console.log(data);
    //[institution, className, classId, choiceText]
    var choiceArray = data[3].split('\n');
  ...
  deletePoll: function(pollId){
    ...
    if(pollUserId === currentUserId) {
      console.log("User deleted a study.")
      Polls.remove({_id: pollId});
    } else if(Roles.userIsInRole(Meteor.user(), ['admin'])){
      console.log("Admin deleted a study.")
      Polls.remove({_id: pollId});
    } else {
      console.log("Someone's trying to delete a study and shouldn't be.")
    }
  },
});
